...
while not state.is_terminal():
    ...
    legible_actions = [
        state.action_to_string(current_player, action) for action in legal_actions
    ]
    ...
    if state.is_chance_node():
        ...
        action = np.random.choice(
            action_list, p=prob_list
        )  # Randomly sample an action according to probs.
        ...
        action = legal_actions[
            0
        ]  # Hardcoded, but this is where your model chooses the action.
